...
class InvoiceServiceTests {
	...
	void testCreateRecurringInvoice() {
		...
		def expectedOther = fromFmt.format(expectedTxnDate) + ' - ' + toFmt.format(new ZonedDateTime(expectedTxnDate).
	...
	void testCreateTimeInvoice() {
		...
		def beginningOfLastMonth = new ZonedDateTime().minusMonths(1).withDayOfMonth(1).toDate()
