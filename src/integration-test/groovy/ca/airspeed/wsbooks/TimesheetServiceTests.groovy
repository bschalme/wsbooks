...
class TimesheetServiceTests {
	...
	static void setUpBeforeClass() {
		...
		jobXref = new TsheetsJobcodeXref()
		jobXref.jobName = 'Test Customer and Job 4'
		jobXref.tsheetsJobcodeId = 42724683
		jobXref.qbCustomerListId = '7F0000-1069293389'
		jobXref.qbItemServiceListId = '140000-1069940598'
		jobXref.save(flush: true, failOnError: true)
	}
    ...
    void testFetchFromTsheets() {
		...
		ctl.tsheetsLastFetchedDate = new ZonedDateTime().withHour(0).withMinute(0).withSecond(0).minusDays(2).toDate()
		...
		assert ctl.tsheetsLastFetchedDate == new ZonedDateTime().withHour(0).withMinute(0).withSecond(0).minusDays(1).toDate()
    }
}
